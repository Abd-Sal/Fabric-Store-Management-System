...
public class SearchProductBillByCodeValidationsEdgeCasesTests

{
    ...
    [Fact]
    public void Should_Have_Error_When_Code_Contains_Only_Spaces()
    {
        // Arrange
        var request = new SearchProductBillByCodeRequest("   ");
        ...
        // Assert
        result.ShouldHaveValidationErrorFor(x => x.code)
            .WithErrorMessage("رمز المنتج لا يمكن أن يكون مسافات فقط");
    }

    [Fact]
    public void Should_Have_Error_When_Code_Is_Whitespace_And_Less_Than_Minimum()
    {
        // Arrange
        var request = new SearchProductBillByCodeRequest("  ");
    ...
    [Fact]
    public void Should_Have_Error_When_Code_With_Spaces_Is_Less_Than_Minimum_Length()
    {
        // Arrange
        var request = new SearchProductBillByCodeRequest("A ");
    ...
    [Fact]
    public void Should_Not_Have_Error_When_Code_Contains_Arabic_Letters_And_Spaces()
    {
        // Arrange
        var request = new SearchProductBillByCodeRequest("منتج تجريبي");
    ...
    [Theory]
    [InlineData("", false, "رمز المنتج مطلوب")]
    [InlineData(null, false, "رمز المنتج لا يمكن أن يكون فارغاً")]
    [InlineData("   ", false, "رمز المنتج لا يمكن أن يكون مسافات فقط")]
    [InlineData("A", false, "رمز المنتج يجب أن يكون على الأقل حرفين")]
    [InlineData("AB", true, null)]
    [InlineData("منتج", true, null)]
    [InlineData("ABC-123", true, null)]
    [InlineData("منتج-123", true, null)]
    [InlineData("ABC_123", true, null)]
    [InlineData("منتج 123", true, null)]
    [InlineData("ABC@123", false, "رمز المنتج يجب أن يحتوي فقط على أحرف عربية وإنجليزية وأرقام وشرطات (- و _) ومسافات")]
    [InlineData("ABC#123", false, "رمز المنتج يجب أن يحتوي فقط على أحرف عربية وإنجليزية وأرقام وشرطات (- و _) ومسافات")]
    public void Test_Code_Validation(string? code, bool shouldBeValid, string? expectedErrorMessage)
    {
        // Arrange
